...
**Rule 5.** `scene_will_change` 시그널을 구독하는 것이 허용된 시스템은 **TRC · EchoLifecycleSM · AudioManager 세 가지**다. `PlayerMovement`는 `scene_will_change`를 직접 구독해서는 **안 된다**; PM의 8-var ephemeral state(player-movement.md F.4.2 row #2: 4 coyote/buffer + 4 facing Schmitt flag)는 EchoLifecycleSM의 O6 cascade를 통해서만 클리어된다. AudioManager는 BGM 하드 컷 + DUCKED 상태 클리어 목적으로 독립 구독한다 (audio.md Rule 7).
...
|---|---|---|---|
| `scene_will_change` | `()` (zero args) | TRC (`_buffer_invalidate()`) · EchoLifecycleSM (`_on_scene_will_change()` cascade — `_lethal_hit_latched` + 입력 버퍼 + O8 counter + PM 8-var via PM `_clear_ephemeral_state()` 호출) · AudioManager (`_on_scene_will_change()` — Music 버스 0 dB 복원 + `stop_bgm()`, audio.md Rule 7) | Single-emit per transition (Rule 4); SM은 sole producer (Rule 4) |
...
HUD #13, VFX #14, Camera #3은 SM의 시그널을 **직접 구독하지 않는다** — 각자 TR / Damage / EchoLifecycleSM의 시그널을 독립 구독한다 (의무 분리; SM은 씬 경계 owner이지 presentation 분배 hub가 아님). Audio #4는 예외 — BGM 하드 컷 목적으로 `scene_will_change`를 직접 구독 (Rule 5 허용, audio.md Rule 7).
...
| ~~`scene_post_loaded` 첫 사용 사례 발생~~ ✅ **Fired 2026-05-12 (DEC-SM-9 / OQ-SM-A1 / Camera #3 RR1 PASS)**: signal active per C.3.4 | ~~C.2.1 POST-LOAD 시그널 노출 추가 (Q2 closure)~~ → Applied; AC-H28 added (RR8) | ~~Camera #3 OR Stage #12 GDD~~ |
