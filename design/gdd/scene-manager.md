...
*Cross-doc drift flag*: `time-rewind.md` (Rules 15/16, E-05/06, AC-B2/B3/B5, F.1 row #11, F.4.1) + `state-machine.md` (B.2, C.2.2 O5, F.3 row, line 499)은 **stale `boss_defeated`** 참조를 보유 — damage.md Round 4 LOCK 이전의 carry-over로 추정. **Post-C.1 housekeeping batch 권장**: `boss_defeated` → `boss_killed` 일괄 교체 (time-rewind.md ~8 사이트 + state-machine.md ~5 사이트, 비파괴적 단순 치환).
...
**Rule 14.** `SceneManager.change_scene_to_packed()`를 호출하기 직전에 인자가 현재 로드된 씬과 동일한 `PackedScene` 참조이면서 의도가 "체크포인트 재시작이 아닌 단순 재전이"이면 early-return + `push_warning()`. 동일 씬 재로드가 *체크포인트 재시작의 일부*인 경우는 유효 — 동일성 검사는 `PackedScene` 참조 + intent flag(`is_checkpoint_restart: bool` 인자)로 판단한다.
...
|---|---|---|---|
| `boss_defeated` → `boss_killed` 통일 | time-rewind.md + state-machine.md | TR ~8 + SM ~5 = ~13 단순 치환 | damage.md F.4 (LOCKED + AC-13 BLOCKING)이 single-source 권위. 본 GDD는 `boss_killed` 사용. 따로 응용 GDD 두 곳 일괄 치환 배치 (Session 19 후속 또는 Session 20 housekeeping) |
...
| **POST-LOAD** | < M 틱 | `change_scene_to_packed` 반환 | 새 씬 `_ready()` 체인 실행; SM이 `tree_changed` 시그널 대기; `checkpoint_anchor` 등록 (Rule 8); READY로 진행. **SM-internal: 본 phase는 외부로 시그널 노출 없음** (Q2 deferral — C.3에서 Camera #3 / Stage #12 의무로 기록). |
| **READY** | steady-state | EchoLifecycleSM이 `state_changed(_, &"alive")` emit | 입력 수락; SM IDLE-effective 복귀. |

**Budget formula** (Rule 9 비협상 계약):

```
SWAPPING(M ticks) + POST-LOAD(< M ticks) + 1(READY 확인) ≤ 60 ticks (= 1.000 s @ 60 Hz)
```

PRE-EMIT은 SWAPPING의 첫 틱과 **co-tick**(Rule 4: `scene_will_change()` emit + `change_scene_to_packed()` 호출이 같은 물리 틱 T에서 발생) — 따라서 PRE-EMIT은 예산에 가산되지 **않는다**. M의 실측 값은 Godot 4.6에서 미검증; Section H AC가 헤드리스 smoke check + Steam Deck 1세대 실측 의무를 보유한다.
...
| T+M+K | SM | `tree_changed` 시그널 감지 → POST-LOAD phase 진입 → `checkpoint_anchor` 등록 (Rule 8) |
...
**Same-PackedScene reload semantics** (Rule 14 보호): 체크포인트 재시작은 동일 PackedScene을 재로드하므로 Rule 14 no-op 가드가 `is_checkpoint_restart=true` intent flag로 통과시킨다.
...
ADR-0003 process priority ladder가 강제하는 같은-틱 emit 순서는 Damage(2) → Boss host(10). 만약 보스 최종 hit 직후 ECHO가 같은 틱에 DYING → DEAD 전이로 죽으면 SM의 핸들러 큐에 두 시그널이 모두 도착할 수 있다:
...
| `damage.md` | F.4 row `boss_killed` "#2 Scene Manager (스테이지 클리어 트리거)" — *(미작성)* annotation 제거 | F.4 row |
...
| `docs/registry/entities.yaml` | 새 constants 2종: `restart_window_max_frames = 60` (= 1.000 s @ 60 Hz Pillar 1 비협상 — Rule 9 contract); `cold_boot_max_seconds = 300` (Pillar 4 5분 룰 — Rule 13 contract) | constants section |
...
**Cross-doc drift housekeeping** (C.1 Rule 12 discovery): `boss_defeated` → `boss_killed` 일괄 치환 — time-rewind.md ~8 사이트 (Rules 15/16, E-05/06, AC-B2/B3/B5, F.1 row #11, F.4.1) + state-machine.md ~5 사이트 (B.2, C.2.2 O5, F.3 row #11, line 499). damage.md F.4 LOCKED + AC-13 BLOCKING이 single-source 권위. **Phase 5d 외 별도 housekeeping 배치 권장** (~13 단순 치환, 비파괴적; Session 19 후속 commit 또는 Session 20).
...
respawn_position = undefined → SM panic state    (if N = 0)
```
...
| result | Vector2 \| panic | — | N ≥ 1 → Vector2; N = 0 → SM panic state (E.1 처리). |
...
**Edge**: `N = 0` → panic state (undefined behaviour 아님). `N > 1`은 level design warning이며 hard error 아님 (mid-stage + stage-start 다중 anchor 배치는 iteration 중 유효한 패턴일 수 있음; warning은 non-fatal).
...
| `new_boundary_state` | BoundaryState enum | {CLEAR_PENDING, RESTART_PENDING, ACTIVE, BOOT_INTRO} | 모든 핸들러 발화 후 SM boundary state. C.2.1 PRE-EMIT entry로 직결. |
| result | BoundaryState | (enum — 4 values) | Deterministic; no RNG. |

**Output Range**: Enum — 4 discrete values. No clamping; 틱당 정확히 1회 설정.
...
**Edge**: 같은 틱에 `dead_seen` 핸들러가 `boss_killed_seen`보다 *먼저* 발화하는 경우 (ADR-0003 priority ladder는 Damage=2 → enemies/Boss=10이므로 EchoLifecycleSM의 `state_changed`는 보스 host의 `boss_killed`보다 앞서 발화) SM의 `_on_state_changed`는 `RESTART_PENDING` 전이를 commit하기 전 `_boundary_state`를 재평가해야 한다. C.3.3 GDScript pattern의 `_boundary_state == CLEAR_PENDING` early-return 가드가 본 케이스를 처리한다.
...
| damage.md | `boss_killed` emit → "#2 Scene Manager (스테이지 클리어 트리거)" | F.4 row line 832 | ✅ Present (미작성 annotation to remove per C.3.5) |
...
| input.md | input event → game start (#2 Scene Manager owner) | line 766 | ✅ Present (no edit needed) |
...
- `damage.md` F.4 row `boss_killed` — *(미작성)* 제거
...
- `docs/registry/entities.yaml` — 2 new constants (`restart_window_max_frames=60`, `cold_boot_max_seconds=300`)
- `design/gdd/systems-index.md` row #2 — In Design → Designed (Phase 5d) → Approved (post-review)

**Cross-doc drift housekeeping** (C.1 Rule 12 discovery): `boss_defeated` → `boss_killed` 일괄 치환 — time-rewind.md ~8 사이트 + state-machine.md ~5 사이트. damage.md F.4 LOCKED + AC-13 BLOCKING이 single-source 권위. Session 19 후속 housekeeping batch로 분리 (~13 단순 치환, 비파괴적).
...
- **`intro_screen_duration_seconds` × `cold_boot_elapsed_s` 누적**: `intro_screen_duration` + 엔진 부트 시간 + 첫 input 대기 시간이 함께 `cold_boot_max_seconds = 300` 예산 안에 들어와야 함. Tier 1 실측 예상: 엔진 부트 ~3 s + intro 8 s + input ~0 s = ~11 s ≪ 300 s ceiling.
...
|---|---|---|
| Logic (automated unit test — GUT) | 12 | BLOCKING |
...
Scene Manager owns no presentation surface — Visual/Feel and UI counts are 0 by design (see UI Requirements section). Two ACs are `[MANUAL]` due to Steam Deck 1세대 hardware dependency; all others are automatable in CI.
...
- **Given** the file `src/autoload/scene_manager.gd` at HEAD
- **When** CI runs `grep -nE '_tokens\s*(=|\+=|-=)' src/autoload/scene_manager.gd`
...
- **Given** the files `src/autoload/scene_manager.gd` and `src/gameplay/player_movement.gd` at HEAD
- **When** `tools/ci/sm_static_check.sh` runs the following three grep patterns:
  1. `grep -nE '(_write_head|_buffer_primed|_buffer_invalidate)' src/autoload/scene_manager.gd` → 0 matches (Rule 6)
  2. `grep -nE 'scene_will_change' src/gameplay/player_movement.gd` → 0 matches (Rule 5)
  3. `grep -nE '\.boot\s*\(' src/autoload/scene_manager.gd` → 0 matches (Rule 10)
...
- **Given** `src/autoload/scene_manager.gd` at HEAD
- **When** `grep -n 'add_to_group.*scene_manager' src/autoload/scene_manager.gd` runs
...
- **Given** `src/autoload/scene_manager.gd` at HEAD
- **When** `grep -nE 'func (_process|_physics_process)' src/autoload/scene_manager.gd` runs
...
- **When** `change_scene_to_packed(S, is_checkpoint_restart=false)` is called
- **Then** `push_warning` fires, no `scene_will_change` is emitted, no scene swap occurs

**Test mechanism**: GUT unit test `test_same_scene_non_restart_noop_and_warns` — assert emit counter = 0 and swap not called.

---

**AC-H13 — Same-scene checkpoint restart intent bypasses no-op guard**
**Classification**: Logic — BLOCKING
**Covers**: Rule 14 (intent flag pass-through)

- **Given** SM has current scene `S` loaded
- **When** `change_scene_to_packed(S, is_checkpoint_restart=true)` is called
...
- **Then** `push_error` fires with "null PackedScene", `_phase` remains IDLE, no `scene_will_change` is emitted

**Test mechanism**: GUT unit test `test_null_packed_scene_panics_no_emit`.
...
- **Given** SM is configured with `debug_simulate_load_failure = true` (forces M+K+1 > 60)
...
**Test mechanism**: GUT integration test `test_budget_exceeded_warns_and_continues` — set debug flag, trigger restart, advance frames past 60, assert warning fired and READY eventually reached.
...
- **Given** `src/autoload/scene_manager.gd` at HEAD
- **When** `grep -nE '(change_scene_to_file|load_threaded_request|load_threaded_get|await)' src/autoload/scene_manager.gd` runs
...
| Edge | AC | Status |
|---|---|---|
| E.1 null PackedScene | AC-H14 | Covered |
...
| `docs/registry/entities.yaml` | C.3.5 · F.4.1 | F.4.1 new constants 2종 (restart_window_max_frames=60 / cold_boot_max_seconds=300) |
...
|---|---|---|
| Godot 4.6 SceneTree API | C.1 R2/R3 · C.2.1 · D.1 · E.3 | `change_scene_to_packed` / `tree_changed` / autoload subsystem |
...
| **qa-lead** | Section H Acceptance Criteria draft | 25 ACs (12 Logic + 11 Integration + 2 `[MANUAL]`); 23 BLOCKING / 2 ADVISORY; B-Inv-1/2/3 1:1 covered; all 14 Rules + 6 Formulas covered; H.U1–H.U3 untestable surfaced. **Note**: qa-lead wrote H directly to file violating Draft→Approval→Write protocol; user reviewed post-hoc and accepted content. |
...
- Last Updated narrative: "Session 19 GDD authoring complete — 25 ACs (23 BLOCKING / 2 ADVISORY), 3 carried OQs resolved (OQ-4 / OQ-PM-1 / OQ-SM-2), 5 new SM-specific OQs registered (Tier 2 deferred), cross-doc drift `boss_defeated → boss_killed` follow-up housekeeping batch queued."
