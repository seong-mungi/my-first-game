...
| **POST-LOAD** | `K` 틱 (design guidance: `K < M`; D.1 binding constraint는 sum) | `change_scene_to_packed` 반환 | 새 씬 `_ready()` 체인 실행; SM이 `tree_changed` 시그널 대기; `checkpoint_anchor` 등록 (Rule 8); READY로 진행. **SM-internal: 본 phase는 외부로 시그널 노출 없음** (Q2 deferral — C.3에서 Camera #3 / Stage #12 의무로 기록). |
...
|---|---|---|
| Logic (automated unit test — GUT) | 15 | BLOCKING |
...
| Rule 2 | AC-H7 |
| Rule 3 | AC-H24 |
| Rule 4 | AC-H3a, AC-H8, AC-H26 (panic-state no-emit guard) |
| Rule 5 | AC-H5, AC-H15 |
| Rule 6 | AC-H5, AC-H16 |
| Rule 7 | AC-H4, AC-H16 |
| Rule 8 | AC-H3b, AC-H9 |
