...
> **Priority**: MVP (Tier 1)
> **Status**: Designed (Lean review skipped — fresh-session `/design-review` 권장)
...
#### C.1.5 디버그 지원 (Tier 2 옵션)

- `state_changed` 시그널을 구독하는 디버그 오버레이 라벨 — 화면 우상단에 `[ECHO] ALIVE → DYING (frame 12345)` 표시
- `_state_history: Array[StringName]` — 마지막 N(=32)개 전이를 ring buffer로 보관. 1000-cycle 결정성 테스트의 *전이 시퀀스 비교* 도구로 사용
- 두 기능 모두 Tier 2 게이트 후 추가. Tier 1은 `print_debug` 한 줄로 대체.
...
```
ECHO (CharacterBody2D, process_physics_priority=0)
├── PlayerMovement (Node — 이동 로직 + 자체 sub-StateMachine 보유; Player Movement GDD 소관)
├── EchoLifecycleSM (StateMachine — 본 프레임워크 인스턴스)
│   ├── AliveState (State)
│   ├── DyingState (State)
│   ├── RewindingState (State)
│   └── DeadState (State)
└── ... (HitBox, AnimationPlayer, etc.)
```
...
func _ready() -> void:
    ...
    var damage := host.get_node(^"Damage") as Damage
    damage.player_hit_lethal.connect(_on_damage_lethal)

    var trc := get_tree().get_first_node_in_group(&"time_rewind_controller")
...
- connect 순서는 코드 내 *기록된 순서를 따른다*. 자동 정렬, 동적 순서 변경 금지.
...
| `B` | int | 2–8 (knob), default 4 | 입력 버퍼 pre-hit window 길이 (frames) | time-rewind.md `RewindPolicy.input_buffer_pre_hit_frames` |
...
| `Q_cap` | int | 4 (const) | transition_to 큐 최대 길이 | C.1.4 결정 |
...
```
input_in_window := (F_input >= F_lethal - B) ∧ (F_input <= F_lethal + D - 1)
```
...
**Edge**: `F_input == -1` (입력 미발생) 시 predicate는 무조건 false.
...
if Q_depth_new > Q_cap:
    push_error("StateMachine cascade — Q_depth exceeded cap (= 4)")
...
| `B` | knob | RewindPolicy 리소스 | 2–8 | Time Rewind GDD (Section G) |
| `R` | const | `REWIND_SIGNATURE_FRAMES` | hardcoded 30 | Time Rewind GDD (불변) |
| `Q_cap` | const | StateMachine 클래스 const | hardcoded 4 | **본 GDD (Section G)** |
...
D-glossary는 framework 외에서 *변경 불가*. 본 GDD는 Q_cap, connect 순서, pause_swallow 화이트리스트만 소유.

---

## E. Edge Cases

본 섹션은 framework가 *명시적*으로 처리해야 할 14개 edge case를 4개 카테고리로 정리한다. 각 항목은 *어떻게 처리하는가*를 동작 단위로 기술하며, "graceful하게 처리"와 같은 모호한 표현은 사용하지 않는다.
...
| **#6 Player Movement** | PlayerMovementSM | ECHO 자식 (별도 SM 노드) | Tier 1 | Player Movement GDD가 7-9 movement 상태 정의. EchoLifecycleSM과는 *독립* (Tier 1 flat 컴포지션) |
...
| F.2 (SM → 의존됨) | #6 Player Movement | `design/gdd/player-movement.md` | **미작성** |
...
### G.1 Framework Owned Knobs (3개)

| Knob | 위치 | 타입 | Safe Range | 기본값 | 영향 받는 게임플레이 측면 | 변경 시 재검증 |
|---|---|---|---|---|---|---|
| **`Q_cap`** | `StateMachine.gd` 클래스 const | int | 2–8 | **4** | 시그널 cascade 발견 임계점. 낮으면 일반 게임플레이 false positive(Tier 2 적 archetype enter()의 audio/animation cue가 잡힘). 높으면 silent 무한 루프 마스킹. | 1000-cycle 결정성 테스트 + Tier 2 적 archetype 5종 통과 시 재측정. |
| **`pause_swallow_states`** | `EchoLifecycleSM` export var | `Array[StringName]` | 0–4 entries (ECHO 머신은 4 state) | `[&"DyingState", &"RewindingState"]` | DYING/REWINDING 중 일시정지 가능 여부 = "분석 시간이 아닌 분노 시간" anti-fantasy 보존. 빈 배열로 두면 어느 상태에서나 pause 허용 — Tier 1 *Easy 모드* knob 후보(Tier 2 결정). | Time Rewind GDD AC 검증 시 동시 검증. |
...
| **`B` (input_buffer_pre_hit_frames)** | `RewindPolicy` resource (Time Rewind GDD Section G) | 입력 버퍼 윈도우 predicate 시작점 (D.2) | Time Rewind GDD 소유. 본 GDD는 2–8 범위 호스팅. |
...
| Tier | 활성 Knob | 비고 |
|---|---|---|
| **Tier 1 (4-6주)** | `Q_cap=4`, `pause_swallow_states=[DyingState, RewindingState]` | `debug_overlay_enabled=false` (`print_debug` 한 줄로 대체) |
...
| **Tier 3 (16개월 출시)** | 추가 knob 0 (Easy 모드는 *별도 시스템* #20 Difficulty Toggle이 `pause_swallow_states`를 빈 배열로 *override*하는 방식 — framework knob 추가 없음) | Difficulty Toggle GDD가 본 GDD knob을 *읽기-쓰기* 가능 클라이언트로 등록 |
...
총 26개 acceptance criteria. *Logic*(GUT 자동 테스트, BLOCKING)이 23개, *Integration*(통합 테스트 또는 문서화된 플레이테스트, BLOCKING)이 3개. 시각/UI ACs 0개 — Foundation 시스템이라 visual evidence 부재. 모든 AC는 Tier 1 prototype 종료 시점까지 PASS해야 한다.
...
### H.4 ECHO Machine Hosting (AC-14 ~ AC-19, AC-26)

| # | Acceptance Criteria | Verification Method | Type |
|---|---|---|---|
| **AC-14** | `EchoLifecycleSM`은 `_ready()`에서 시그널을 다음 *정확한* 순서로 connect한다: (1) Damage.player_hit_lethal (2) TRC.rewind_started (3) TRC.rewind_completed (4) TRC.rewind_protection_ended (5) SceneManager.scene_will_change. 순서 변경 시 본 AC 실패. | GUT: connect 호출을 spy로 가로채 순서 캡처 | Logic |
| **AC-15** | O1 latch: `ALIVE → DYING` 전이 직후 `_lethal_hit_latched == true`. `REWINDING → ALIVE` 또는 `→ DEAD` 전이 직후 `false`. 다른 전이에서는 변화 없음. | GUT: 4 전이 시나리오 각각에서 latch 값 검증 | Logic |
...
| **AC-17** | O3 입력 버퍼: `D=12, B=4` 기본값에서 D.2 표의 5개 시나리오가 모두 정확히 판정된다 (in/out). | GUT: F_lethal 고정 + F_input 5 case → predicate 평가 | Logic |
...
| **AC-22** | E-14 arity 불일치: SM 핸들러가 시그널 시그니처와 인자 수가 다를 때 emit 시 (a) Godot 콘솔 에러 발생 (b) latch + 전이 0회 변화. | GUT: 의도적으로 arg 수 불일치 → emit → SM 상태 무변화 검증 | Logic |
...
| **AC-25** | C.3.6 성능 budget: ECHO + 30 적 + 1 보스 = 32 동시 SM이 가동되는 1 stage 기준 60fps 5분 플레이테스트에서 frame time profile 상 SM 호출 누적 < 0.5ms / frame. | Integration: Godot profiler 기록 + per-frame breakdown screenshot | Integration |

### H.7 Tier별 Gate

| Tier | 통과 필수 AC |
|---|---|
| **Tier 1 (4-6주)** | AC-01 ~ AC-26 모두 (AC-26 DEAD 입력 무응답 포함) |
| **Tier 2 (6개월)** | + 적 archetype 3종 추가에서 AC-23 재검증 + 1000-cycle 결정성 (AC-24) auto-CI 통합 |
| **Tier 3 (16개월)** | + Difficulty Toggle 통합 시 `pause_swallow_states` override 검증 (Difficulty Toggle GDD 소관) |
...
├── echo_lifecycle_sm_test.gd             # AC-14 ~ AC-19, AC-26
...
| ID | 질문 | 결정 시점 |
|---|---|---|
| **OQ-SM-4** | `pause_swallow_states` 빈 배열 override (Easy 모드 옵션) — Tier 1 prototype에 포함할지 vs Tier 2 Difficulty Toggle GDD 작성 시점까지 보류? Easy 토글의 기본 인터페이스 결정과 함께 평가. | Tier 1 Week 4 + Difficulty Toggle GDD 작성 게이트 |
| **OQ-SM-5** | `Q_cap=4` 기본값이 Tier 2 적 archetype 5종 추가 시점에도 false positive 없이 유지되는가? Tier 2 게이트에서 실측 후 6 또는 8로 상향 결정. | Tier 2 게이트 |
...
- **(해소)** `Q_cap` 기본값 — `4` (G.1). 솔로 Tier 1에 적합.
