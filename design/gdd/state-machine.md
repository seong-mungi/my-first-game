...
| **O6** | `scene_will_change` 도착 시 SM이 `_lethal_hit_latched` + 입력 버퍼 변수 + O8 idempotency 카운터를 클리어 | time-rewind.md Rule 16(전제) + E-16 + input.md C.1.5 (O7/O8 cascade) | `EchoLifecycleSM`이 Scene Manager(#2)의 `scene_will_change` 시그널을 구독. 핸들러가 모든 ephemeral 변수를 리셋(`_lethal_hit_latched = false`, `_rewind_input_pressed_at_frame = -1`, `_session_first_success_emitted = false` — O8 cascade per input.md C.1.5 latch-reset 의무). 현 state는 변경하지 않음(다음 씬 부트 시 Scene Manager가 ALIVE로 강제 전이). |
