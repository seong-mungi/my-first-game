...
2. **UI가 깜빡이는 순간이 없다.** REWINDING 도중 `boss_killed`가 도착해도 HUD 토큰 카운터는 즉시 갱신되지 않고 `rewind_protection_ended`까지 지연된다. 즉, 같은 0.5초 안에 시각 시그니처가 두 번 바뀌는 일이 없다. 이 지연 정책의 enforcer는 SM이다.
...
ECHO의 4-state 머신(ALIVE / DYING / REWINDING / DEAD)은 본 프레임워크의 *첫 번째 클라이언트*다. 단일 출처 원칙에 따라 4-state 머신 자체의 *행동 정의*는 [`design/gdd/time-rewind.md`](time-rewind.md) Section C가 보유하며, 본 GDD는 이 머신이 *프레임워크에 부과한 9가지 의무*(O1-O6 원본 + O7/O8 Input #1 B15 + O9 Input #1 B14/O-IN-1, 둘 다 Round-7 exception 2026-05-11)만 명시한다. 의무 위반은 Time Rewind GDD AC를 깨므로, 본 프레임워크는 모든 의무를 *호스팅 가능*해야 한다.
...
- **외부 polling 메서드**: `should_swallow_pause() -> bool` (Pause 시스템 #18이 호출) **+ alias `can_pause() -> bool`** (Input #1 PauseHandler autoload이 호출 — input.md C.1.4 단일 출처). 두 메서드는 *공존 alias*: `func can_pause() -> bool: return not should_swallow_pause()`. Round-7 cross-doc-contradiction exception 적용 (Input #1 F.4.1 #7 closure 2026-05-11).
...
#### C.2.2 9가지 의무 (Time Rewind이 SM에 부과; O7/O8은 Input #1 B15 + O9는 Input #1 B14/O-IN-1 — 둘 다 Round-7 cross-doc-contradiction exception 2026-05-11)
...
| **O2** | DYING/REWINDING 상태에서 `pause` 입력 swallow | time-rewind.md Rule 18 + input.md C.1.4 PauseHandler 패턴 (alias `can_pause()` 의무) | `EchoLifecycleSM`이 `pause_swallow_states: Array[StringName] = [&"DyingState", &"RewindingState"]` 화이트리스트를 보유. **두 alias method 공존**: (a) `should_swallow_pause() -> bool` — 기존 SM 측 호출자(Pause 시스템 #18 future) 위한 inverse-polarity API; 현 state name이 화이트리스트에 있으면 true 반환; (b) `can_pause() -> bool` — Input #1 PauseHandler autoload이 호출하는 positive-polarity API (input.md C.1.4 단일 출처); 구현은 `func can_pause() -> bool: return not should_swallow_pause()` 1-line. **Input #1 F.4.1 #7 closure 2026-05-11** (Round-7 cross-doc-contradiction exception). 두 호출자 모두 공존 보장 — rename 아닌 alias 추가로 backward-compat 유지. |
...
| **O5** | `boss_killed` REWINDING 중 도착 시 토큰 ++ 처리는 *TRC* 책임이지만, HUD 시각 갱신 지연(`rewind_protection_ended`까지)은 SM이 보장 | time-rewind.md Rule 15 | RewindingState가 `boss_killed` 시그널을 *직접 구독하지 않는다*. TRC가 토큰 카운트를 갱신하고 `token_replenished`를 emit하지만 HUD가 그 시점에 즉시 갱신하지 않도록, HUD 자체가 `rewind_protection_ended` 발생 후 적용하는 buffered-update 패턴을 사용 (HUD GDD 소관 — 본 GDD는 *제약*만 declare). |
| **O6** | `scene_will_change` 도착 시 SM이 `_lethal_hit_latched` + 입력 버퍼 변수 + O8 idempotency 카운터 + **PM 8-var ephemeral state** (via PM `_clear_ephemeral_state()` 호출) 클리어 — single-layer cascade design | time-rewind.md Rule 16(전제) + E-16 + input.md C.1.5 (O7/O8 cascade) + scene-manager.md C.1 Rule 5 + C.3.2 T+0 row + player-movement.md F.4.2 row #2 | `EchoLifecycleSM`이 [Scene Manager #2](scene-manager.md)의 `scene_will_change` 시그널을 구독 (canonical contract — scene-manager.md C.1 Rule 4 sole producer, Rule 5 sole subscribers TRC + EchoLifecycleSM; PM 직접 구독 금지). 핸들러가 모든 ephemeral 변수를 리셋(`_lethal_hit_latched = false`, `_rewind_input_pressed_at_frame = -1`, `_session_first_success_emitted = false` — O8 cascade per input.md C.1.5 latch-reset 의무) **+ PM `_clear_ephemeral_state()` 호출** (PM 8-var: 4 coyote/buffer + 4 facing Schmitt flag clear — player-movement.md F.4.2 row #2). 현 state는 변경하지 않음(다음 씬 부트 시 [Scene Manager #2](scene-manager.md)가 자연 `_ready()` 체인으로 ALIVE 강제 전이 — scene-manager.md C.1 Rule 10). **Provisional → confirmed 2026-05-11 (Scene Manager #2 Approved RR7)**. |
| **O7** | `first_death_in_session(profile: StringName)` 시그널 emit — **매** `ALIVE → DYING` 전이 시 (recurring, NOT 1-time latch) | input.md C.1.5 + B15 (2026-05-11) — **Round-7 cross-doc-contradiction exception** applied per `damage.md` Round 5 S1 BLOCKER pattern (Session 6 2026-05-10) | `EchoLifecycleSM`이 `transition_to(DyingState)` atomic completion 직후 `first_death_in_session.emit(active_profile)`. SM 측 latch *없음* — HUD가 자체 latch 소유 (input.md C.1.5 `_first_rewind_success_latched`). `active_profile`은 ActiveProfileTracker autoload polling (input.md C.1.6). dispatch_signal 가드 D.3 적용 대상 *아님* (이 emit은 외부 입력 reactive가 아닌 SM 자체 transition observation). |
| **O8** | `first_rewind_success_in_session(profile: StringName)` 시그널 emit — 세션 내 최초 `REWINDING → ALIVE` 전이 + lethal-hit-driven (`_lethal_hit_latched_prev == true`) 시 1회 | input.md C.1.5 + B15 (2026-05-11) — **Round-7 cross-doc-contradiction exception** applied | `EchoLifecycleSM`이 멤버 `_session_first_success_emitted: bool = false`를 추가 소유. `REWINDING → ALIVE` 전이 atomic completion 직후 평가: O1 latch clear 직전 1-frame 보존본 `_lethal_hit_latched_prev` (intra-tick ordering — clear *직전* 캡처) 사용 → `if _lethal_hit_latched_prev AND NOT _session_first_success_emitted: first_rewind_success_in_session.emit(active_profile); _session_first_success_emitted = true`. hazard-grace 자연 회복(`REWINDING → ALIVE` w/o lethal hit, `_lethal_hit_latched_prev == false`)은 emit 안 함 (Pillar 1: 첫 *진짜* 성공만 학습 증명). O6 cascade가 `_session_first_success_emitted = false`로 리셋. |
| **O9** | `_trigger_held: bool` gate — `JOY_AXIS_TRIGGER_LEFT` threshold 0.5 근처 chatter (Steam Deck 1세대 LT 마모 시) 흡수: `is_action_just_pressed("rewind_consume")` 한 hold 사이클당 1회 발화 보장 | input.md F.4.1 O-IN-1 + B14 (2026-05-11) — **Round-7 cross-doc-contradiction exception** applied per `damage.md` Round 5 S1 BLOCKER pattern (Session 6 2026-05-10) | `EchoLifecycleSM`이 멤버 `_trigger_held: bool = false`를 추가 소유. AliveState/DyingState `physics_update`에서 O3 입력 버퍼(`_rewind_input_pressed_at_frame`) 갱신 *직전* 가드: `if Input.is_action_pressed(&"rewind_consume") and not _trigger_held: _trigger_held = true; _rewind_input_pressed_at_frame = Engine.get_physics_frames()` · `elif not Input.is_action_pressed(&"rewind_consume"): _trigger_held = false`. axis 진동(0.45↔0.55)이 `is_action_just_pressed` 재발화를 일으켜도 게이트가 한 hold당 1회만 진입 허용. KB+M `Shift` 경로는 `InputEventKey` 이진 source이므로 chatter 없음 — gate는 효과 없이 정상 통과. O6 cascade가 `_trigger_held = false`로 리셋. dispatch_signal 가드 D.3 영향 없음 (이 gate는 *외부 input polling level*에서 작동, framework signal 라우팅과 무관). |
...
| 핸들러 미구현 | false | DyingState | `boss_killed` | 가드 3에서 return |
...
| **#2 [Scene / Stage Manager](scene-manager.md)** | `scene_will_change()` (0 args) | `_on_scene_will_change` | latch + 입력 버퍼 ephemeral 변수 + O8 idempotency counter + PM `_clear_ephemeral_state()` 호출 (8-var cascade) 클리어 (E-11 + O6) | **Scene Manager #2 Approved RR7 2026-05-11** — canonical contract locked: scene-manager.md C.1 Rule 4 (sole producer) + Rule 5 (sole subscribers TRC + EchoLifecycleSM; PM 직접 구독 금지) + C.3.2 T+0 cascade wiring; `*provisional*` removed. |
| **#1 Input System** | (시그널 아님 — 폴링) `Input.is_action_just_pressed("rewind_consume")` | AliveState/DyingState `physics_update`에서 폴링 | `_rewind_input_pressed_at_frame` 갱신; LT chatter hysteresis는 신규 `_trigger_held` gate 의무 (input.md C.5 + E-IN-2 cross-doc obligation 2026-05-11) | Input GDD [`design/gdd/input.md`](input.md) **Designed 2026-05-11** — `InputMap` action 이름 verbatim lock (`rewind_consume`). `_trigger_held` gate 추가 의무: AliveState/DyingState `physics_update`에서 `is_action_just_pressed` 이후 `is_action_just_released` 까지 동일 hold 동안 재발화 무시 (Steam Deck 1세대 LT 마모 chatter 흡수). |
...
| **[#7 Player Shooting / Weapon System](player-shooting.md)** | *(no SM — read-only subscriber)* | WeaponSlot Node2D, PM root 자식 | Tier 1 | **Player Shooting #7 Approved 2026-05-11 Round 2 (read-only state subscriber, NOT framework client)**: WeaponSlot은 자체 StateMachine을 호스팅하지 *않으며* (player-shooting.md C.2.1 — read-driven member-var + guard ladder 패턴), `EchoLifecycleSM.state_changed(from, to)` 시그널만 구독하여 `_active: bool` toggle 단일 출처로 갱신 (C.2.3 `_on_lifecycle_state_changed` handler — `AliveState`/`RewindingState` → `_active = true`; `DyingState`/`DeadState` → `_active = false`). `lifecycle_sm.current_state` read for G2 guard (정의된-깊이 가드 — G1 mirror). **`transition_to()` 호출 금지** (forbidden_pattern `cross_entity_sm_transition_call` 정합; GREP-PS-PS-H6-05 static check enforces 0 matches under `src/gameplay/player_shooting/`). 본 row는 F.2의 *read-only state subscriber* 패턴 첫 등재 — 향후 HUD #13 / Camera #3 / VFX #14 등 SM 시그널만 read하는 시스템도 동일 패턴으로 등재. |
...
| **[#3 Camera System](camera.md)** | *(no SM — read-only state subscriber)* | Camera2D, stage root sibling | Tier 1 | **Camera #3 Approved 2026-05-12 RR1 PASS (read-only state subscriber, NOT framework client)**: Camera는 자체 StateMachine을 호스팅하지 *않음* (camera.md C.2 — single logical state `FOLLOWING` + 2 bool flags `is_rewind_frozen` / `apply_snap_next_frame`; framework 오버헤드 미정당). PlayerMovementSM `state` direct read for vertical lookahead state-scaled lerp (R-C1-4 — IDLE/RUN → 0; JUMPING → −20; FALLING → +52; REWINDING/DYING → 즉시 0 클램프). EchoLifecycleSM/PlayerMovementSM `state_changed` 시그널 구독은 Tier 1에서 미사용 (camera.md C.3.1 — Camera는 6 시그널 구독: scene_post_loaded · shot_fired · player_hit_lethal · boss_killed · rewind_started · rewind_completed; SM emit 시그널은 직접 구독 안 함). **`transition_to()` 호출 금지** (forbidden_pattern `cross_entity_sm_transition_call` 정합). 본 row는 F.2의 *read-only state subscriber* 패턴 두 번째 등재 (Player Shooting #7 precedent 정합). |
...
| 시스템 | 영향 형태 | 상호작용 |
|---|---|---|
| **#13 HUD System** | `boss_killed` 토큰 시각 갱신을 `rewind_protection_ended`까지 지연 (C.2.2 O5) | HUD가 SM 시그널 직접 구독 안 함 — TRC의 `token_replenished`만 구독하나 *타이밍 정책*은 SM이 보장 |
...
| **#4 [Audio System](audio.md)** | EchoLifecycleSM → `AudioManager.play_rewind_denied()` direct call | TR AC-B4: `_tokens == 0` + DYING + rewind input → EchoLifecycleSM이 직접 호출. 시그널 없음 (`play_rewind_denied()` 패턴 — audio.md Rule 12). Audio #4 Approved 2026-05-12. |
...
| F.1 (SM ← 의존) | #1 Input System | [`design/gdd/input.md`](input.md) | **Designed 2026-05-11** — Input GDD F.1/F.4.1 cross-doc; `rewind_consume` action name verbatim lock; `_trigger_held` gate 의무 (input.md C.5). 본 SM F.1 row 832 + OQ-SM-3 closure는 Input #1 F.4.1 batch 적용. |
...
| ~~**OQ-SM-2**~~ ✅ **Resolved 2026-05-11 (Scene Manager #2 Approved RR7)** | ~~SM의 `boot()` 호출 책임자 — ECHO 노드의 `_ready()` 안에서 호출(현재 가정)? 아니면 Scene Manager가 ECHO 인스턴스화 직후 명시 호출?~~ → **EchoLifecycleSM이 자체 `_ready()`에서 부트** (씬 트리 자연 부트 per C.2.1 line 322 lock-in). Scene Manager는 `EchoLifecycleSM.boot()`를 **직접 호출하지 않음** (scene-manager.md C.1 Rule 10 sole-source); SM은 `change_scene_to_packed`로 씬 swap만 수행하며 새 씬의 `_ready()` 체인이 자연 부트 트리거. 이중 부트 위험 + 솔로 디버깅 복잡도 회피. | [`design/gdd/scene-manager.md`](scene-manager.md) C.1 Rule 10 | **Resolved** |
| ~~**OQ-SM-3**~~ ✅ **Resolved 2026-05-11** | ~~`Input.is_action_just_pressed("rewind_consume")` 이름이 확정인가?~~ → **`rewind_consume` verbatim lock** (input.md C.1.1 row 8 단일 출처). F.1/D.2/AC-17의 *provisional* 가정 모두 일관 — 별도 변경 불필요. Input #1 F.4.1 #6 closure. | [`design/gdd/input.md`](input.md) C.1.1 row 8 | **Resolved** |
