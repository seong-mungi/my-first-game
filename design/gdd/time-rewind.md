...
ring_buffer_bytes   = W × snapshot_total = 90 × 196 ≈ 17,640 B ≈ 17.2 KB
ceiling_bytes       = 1.5 GB ≈ 1.61 × 10^9 B
budget_fraction     = 17,640 / 1.61e9 ≈ 0.001%
ADR-0002_cap        = 25 KB (Amendment 1)        — 17.2 KB ≪ 25 KB → ✓
```
...
**Output Range**: 17.2–21.4 KB resident (Amendment 2 +0.36 KB delta vs Amendment 1). `_ready()`에서 1회 사전 할당 (per-tick allocation 0). StringName은 Godot 문자열 테이블에 interned되므로 포인터 비용은 8B 일정. **ADR-0002 Amendment 1 25 KB cap 잔여 7.8 KB** — Tier 2 추가 필드 ~5개 여유.

**Example**: 90 슬롯 × 196 B = 17,640 B. TRC 노드 오버헤드(~500 B) 추가 시 ~18.1 KB. 1.5 GB ceiling 대비 0.001%.
