...
> **Last Updated**: 2026-05-13 — warning-level AC coverage cleanup
...
| # | System | Direction | Interface | Hard/Soft |
|---|---|---|---|---|
| #11 | [Boss Pattern](boss-pattern.md) | Emits on boss death through the Damage-compatible boss host; TRC subscribes and self-calls. Direct call prohibited. | `signal boss_killed(boss_id: StringName)` → TRC subscribes and calls `self.grant_token()` → emits `token_replenished(new_total: int)`. Boss Pattern #11 Approved 2026-05-13 mirrors priority 10 and phase rewind-immunity. | **Hard** (token economy) |
| #13 | [HUD](hud.md) | HUD subscribes to TRC signals (observer, no polling). | Approved 2026-05-13: subscribes to `token_consumed(remaining_tokens: int)`, `token_replenished(new_total: int)`, and `rewind_protection_ended(player: Node2D)` for delayed reward display. Initial display via `TimeRewindController.get_remaining_tokens() -> int`. HUD updates only after `_displayed_tokens` diff check (per-fire allocation 0). | **Soft** |
| #14 | [VFX / Particle](vfx-particle.md) | Approved 2026-05-13. VFX subscribes to `rewind_started` and sets eligible forward-causality particles `GPUParticles2D.emitting = false`; `rewind_completed` may re-anchor player-local effects only; `rewind_protection_ended` is the sole resume event. | Subscribes to `rewind_started(remaining_tokens: int)`, `rewind_completed(player: Node2D, restored_to_frame: int)`, and `rewind_protection_ended(player: Node2D)`. ADR-0001 workaround remains: no reverse particle simulation; emit-pause + restart, using `restart(keep_seed=true)` where applicable. Full-screen color inversion / UV glitch remains Shader #16. | **Soft** |
| #16 | [Time Rewind Visual Shader](time-rewind-visual-shader.md) | Approved 2026-05-13. Shader subscribes only to `rewind_started` + runs a protection-aligned internal controller; visible fullscreen intensity is active frames 0–18 and off for frames 19–30. Wiring termination to `rewind_completed` is prohibited (same-tick emit). | Subscribes to `rewind_started(remaining_tokens: int)`. Internal protection-aligned controller; debug-only agreement check against `rewind_protection_ended`. | **Soft** |
...
| #11 Boss Pattern | ✅ Approved 2026-05-13: emits `boss_killed(boss_id: StringName)` through Damage-compatible boss host, explicitly notes phase transition rewind-immunity, and locks `process_physics_priority = 10` | C.3, E-18 |
| #13 HUD | `_displayed_tokens` diff check (per-fire allocation 0), delay visual update during REWINDING | I6, Rule 15 |
| #14 VFX | ✅ Approved 2026-05-13: pause eligible particles on `rewind_started`, use `rewind_completed` only for re-anchor, restart on `rewind_protection_ended`; Shader #16 owns full-screen inversion/glitch. | C.3 |
| #16 Visual Shader | ✅ Approved 2026-05-13: subscribe only to `rewind_started`, run protection-aligned internal controller, visible fullscreen intensity ends by frame 18, and termination from `rewind_completed` remains prohibited. | I5 |
...
| **#15** | [Collage Rendering Pipeline](collage-rendering.md) | Soft (Pillar 1 consistency) | No data flow — *visual readability* dependency | Approved 2026-05-13. Pillar 1 ("punishment→learning tool") consistency holds only when enemies, bullets, and hazards are not obscured by collage layers in a 0.2s glance (game-concept R-D2). Collage #15 specifies the 0.2-second glance gate (AC-COL-02/17) and Shader #16 contrast handoff (AC-COL-11/19). |
...
| **#14** | [VFX / Particle](vfx-particle.md) | Soft | `rewind_started` + `rewind_completed` + `rewind_protection_ended` signals | Approved 2026-05-13. GPUParticles2D emit-pause / re-anchor / restart; no reverse particle simulation; full-screen shader handoff to #16 |
| **#16** | [Time Rewind Visual Shader](time-rewind-visual-shader.md) | Soft | `rewind_started` signal only + protection-aligned internal controller | Approved 2026-05-13. Color-inversion + glitch visual signature; visible fullscreen intensity frames 0–18, inactive tail frames 19–30. |
...
> **D1 fix 2026-05-10 — Silent cap-overflow contract gap (HUD coordination obligation; closed 2026-05-13 by HUD #13 approval)**: When `boss_killed` arrives with `_tokens == max_tokens` (5), `grant_token()` silently clamps — `_tokens` unchanged, `token_replenished` emit fires (Rule 3 / AC-B3 invariant). However, the anti-fantasy "spent an expensive resource — every transaction must be felt" breaks at the cap (boss defeat reward is invisible visually/audibly). **HUD resolution**: `hud.md` Rule 3 and AC-HUD-3 require the `token_replenished` handler to treat `new_total == old_displayed == max_tokens` as separate `cap_full` feedback, not as a gain/no-op. This GDD owns the signal contract; HUD owns the presentation branch. In Tier 1 baseline, the cap-full case can occur one or more times (start with 3 tokens, reach cap after 4-5 boss kills).
...
- **AC-B4b** [AUTO] GIVEN `_tokens > 0`, `_buffer_primed == true`, and ECHO is ALIVE in a normal mid-session scene, WHEN Scene Manager emits `scene_will_change()` in tick T and a same-tick or next-tick lethal hit opens DYING, THEN TRC `_buffer_invalidate()` sets `_buffer_primed == false`, `try_consume_rewind()` returns `false`, token count is unchanged, SM remains in DYING, and `AudioManager.play_rewind_denied()` fires exactly once with denial reason `buffer_invalidated`. Silent return is forbidden in this mid-session invalidation path. _Verifies Rule 7 extended denial / Rule 13-bis / scene-manager.md Rule 15._
...
- **Visual Shader (#16)**: Protection-aligned internal controller wired only to `rewind_started`; *absence* of `rewind_completed` termination trigger (Integration Risk I5). Fullscreen intensity is required to clear by frame 19 per `time-rewind-visual-shader.md`.
