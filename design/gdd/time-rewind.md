...
- **AC-A1** [AUTO] GIVEN 세션이 W(90) 물리 프레임 이상 진행, WHEN `_physics_process` 발화, THEN `_capture_to_ring()`이 9개 `PlayerSnapshot` 필드 (8 PM-노출 + 1 TRC-internal — Amendment 2 2026-05-11)를 `_ring[_write_head]`에 기록하고 `_write_head = (_write_head + 1) % 90`을 같은 틱에 진행. _Rule 1 / Formula D3 검증._
