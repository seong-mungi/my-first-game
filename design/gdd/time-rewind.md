...
- **ADR-0001 (R-T1)** — Player-only scope. ECHO만 되감기 대상이며 적·탄환·환경은 정상 시뮬레이션 계속. 8-field PM-노출 `PlayerSnapshot` 스키마 (Amendment 2 락인 2026-05-11: 7 PM-owned + 1 Weapon-owned `ammo_count`; Resource 전체 9 필드 = 8 PM-노출 + 1 TRC-internal `captured_at_physics_frame` per Amendment 1). 시그널 계약 4종(`rewind_started`, `rewind_completed`, `token_consumed`, `token_replenished`).
...
15. `boss_killed` 시그널이 REWINDING 중 도착하면 `_tokens` 즉시 증가 + `token_replenished` emit. HUD는 `rewind_protection_ended` 까지 시각 갱신을 지연한다(시그니처 중 UI 변동 차단).
16. `boss_killed`와 `lethal_hit_detected`가 같은 물리 틱에 도착하는 경우 — **실제 invariant** (Round 1 design-review 정정): process_physics_priority lower=earlier (Godot 4.x). 사다리: player=0, TRC=1, Damage=2 (damage.md Round 3 lock), enemies/Boss=10, projectiles=20. 따라서 같은 틱 실행 순서는 Damage(2) → Boss(10) → projectile(20). Damage가 먼저 `lethal_hit_detected` emit → SM이 buffered rewind input 보유 시 `try_consume_rewind()` → `_tokens -= 1` (T → T-1, 중간 T=0 dip 가능) → Boss가 `boss_killed` emit → `grant_token()` → `_tokens = min(T+1, max_tokens)` (T-1 → T, net zero). 중간 T=0 dip은 같은 틱 내 비관측 (AC-B5는 net-zero 검증, 중간 상태 검증 안 함). **불변 깨짐 조건**: Boss가 자체 `_physics_process` 노드에서 priority < 2 슬롯 획득 시. Boss Pattern GDD가 priority ≥ 10 상한 의무.
...
| #1 | [Input System](input.md) | InputMap action 매핑. SM이 DYING 진입 시 액션을 폴한다. **Input #1 Designed 2026-05-11**: cross-doc obligations verbatim locked (input.md C.1.1 row 8 + C.3.1 KB+M Shift + C.3.2 Gamepad LT threshold 0.5 + C.2 single-button-no-chord Tier 1 invariant + C.2 Tier 3 `default_only` interpretation — OQ-15 closure). | InputMap action `"rewind_consume"`. 게임패드 좌트리거 (`JOY_AXIS_TRIGGER_LEFT`, threshold 0.5). KB+M 기본 `Shift`. 단일 버튼, 코드 X (technical-preferences 제약). | **Hard** |
| #2 | [Scene / Stage Manager](scene-manager.md) | Scene Manager가 `change_scene_to_packed` 호출 직전 (same physics tick, sync emit) emit; TRC가 자체 `_buffer_invalidate()` 핸들러로 구독해 ring buffer invalidate. | `signal scene_will_change()` (0 args; Scene Manager autoload sole producer per scene-manager.md C.1 Rule 4) → TRC self `_buffer_invalidate()` (TRC ring buffer single owner — SM does not touch `_write_head`/`_buffer_primed` per scene-manager.md C.1 Rule 6): `_buffer_primed = false`, `_write_head = 0`. 토큰 카운트는 보존 (scene-manager.md C.1 Rule 7 + D.2 invariant). 다음 DYING은 buffer-primed 가드(Rule 13-bis)에 의해 자동 차단되어 `DEAD`로 직행. 씬 경계의 invalid 좌표 복원을 방지 (E-16). **OQ-4 closure 2026-05-11 (Scene Manager #2 Approved RR7)**: emit timing = `change_scene_to_packed` 호출 직전 동일 물리 틱 sync emit; token preservation invariant locked. | **Hard** |

#### Downstream

| # | 시스템 | 방향 | 인터페이스 | Hard/Soft |
|---|---|---|---|---|
| #11 | Boss Pattern *(provisional)* | Boss 사망 시 emit; TRC가 구독해 자기 호출. 직접 호출 금지. | `signal boss_killed(boss_id: StringName)` → TRC가 구독해 `self.grant_token()` 호출 → `token_replenished(new_total: int)` emit. | **Hard** (토큰 경제) |
...
ADR-0002_cap        = 25 KB (Amendment 1)        — 17.64 KB ≪ 25 KB → ✓ (잔여 7.36 KB headroom per Amendment 2)
```

**Field 분해** (Amendment 2 2026-05-11 — 9 fields: 8 PM-노출 + 1 TRC-internal):

| Field | GDScript 타입 | Bytes | Owner |
|---|---|---|---|
| `global_position` | Vector2 | 8 | PM |
| `velocity` | Vector2 | 8 | PM |
| `facing_direction` | int | 4 | PM |
| `animation_name` | StringName (포인터) | 8 | PM |
...
**Output Range**: 17.64–22.1 KB resident (Amendment 2 +0.36 KB delta vs Amendment 1). `_ready()`에서 1회 사전 할당 (per-tick allocation 0). StringName은 Godot 문자열 테이블에 interned되므로 포인터 비용은 8B 일정. **ADR-0002 Amendment 1 25 KB cap 잔여 7.36 KB** (Amendment 2 recalc — was 7.8 KB pre-Amendment-2; B1 fix `/review-all-gdds` 2026-05-11) — Tier 2 추가 필드 ~5개 여유.

**Example**: 90 슬롯 × 196 B = 17,640 B = 17.64 KB. TRC 노드 오버헤드(~500 B) 추가 시 ~18.14 KB. 1.5 GB ceiling 대비 0.0012%.
...
```
t_capture_per_tick  ≈ N_fields × t_field_copy = 9 × ~100–500 ns ≈ 0.9–4.5 μs (write-into-place; Round 2 정정 + B2 fix `/review-all-gdds` 2026-05-11 N_fields 8→9 per Amendment 2)
t_restore_one_time  ≈ N_fields × t_field_copy + t_anim_seek
                    ≈ 9 × ~300 ns + 200 μs ≈ 203 μs                          (AnimationPlayer.seek dominate; t_field_copy 영향 미미)
frame_budget        = 16,600 μs (1 / 60 fps)
rewind_subsys_cap   = 1,000 μs  (≤1 ms envelope, technical-preferences.md)
capture_fraction    ≤ 4.5 μs / 16,600 μs ≈ 0.027% per tick                   (worst case — Amendment 2 N_fields=9 + Round 2 정정)
restore_fraction    = 203 μs / 16,600 μs ≈ 1.2% (단발성, rewind 발동 프레임만)
```
...
| 기호 | 타입 | 범위 | 설명 |
|---|---|---|---|
| `N_fields` | int | 9 | PlayerSnapshot 필드 수 (Amendment 2 2026-05-11: 8 PM-노출 + 1 TRC-internal). 이전 표기 8은 Amendment 1 기준 (capture_at_physics_frame 포함). Capture/restore CPU 추정치 변화는 +12% within envelope — 결론 불변. |
...
**Example**: 정상 플레이 60fps에서 캡처 비용 = ~300 ns × 9 fields × 60 ticks ≈ 162 μs/sec → 16.6 ms frame budget × 60 frames/sec = 996,000 μs/sec; 162 / 996,000 ≈ 0.016% (Amendment 2 N_fields=9 + Round 2 정정; B2 fix `/review-all-gdds` 2026-05-11). 1회 rewind 발동: 발동 프레임에 203 μs, ≤ 1 ms 봉투 만족.
...
**F6 — 탄약 카운트 복원 정책 (RESOLVED 2026-05-11 via player-movement.md DEC-PM-3 v2 / B5)**: `PlayerSnapshot`은 무기 ID(`current_weapon_id`)와 **`ammo_count: int` (8번째 PM-노출 필드, Resource 9번째 — Amendment 2)**를 모두 per-tick 캡처하고 복원한다. `restore_from_snapshot(snap)` 후 ECHO의 ammo는 9프레임 전(`restore_idx` 시점)의 값으로 복원. Pillar 1 ("처벌이 아닌 학습 도구") 단일 출처 — rewind가 0-ammo 상태로 복원되지 않음. **Write authority**: `ammo_count`는 PM single-writer policy 적용 *X* — Weapon (#7) single-writer. `PM.restore_from_snapshot(snap)`은 `snap.ammo_count`를 *ignore*; Weapon-side 복원은 OQ-PM-NEW에 따라 (a) TRC orchestration 또는 (b) Weapon이 `rewind_completed` signal 구독 후 자체 restore 중 하나. **ADR-0002 Amendment 2** (2026-05-11 Proposed) 의무. ~~이전 정책 (2026-05-10) "resume with live ammo" + (a)/(b) Weapon GDD 선택 의무는 폐기.~~
...
- **E-05** REWINDING 중 boss_killed → 토큰 즉시 +1, HUD 시각 갱신은 `rewind_protection_ended` 까지 지연 (Rule 15)
- **E-06** boss_killed와 lethal_hit_detected 같은 틱 → consume-then-grant 순서 (Damage priority=2 < Boss priority=10), net-zero, 중간 T=0 dip 비관측 (Rule 16 정정)
...
- **E-22** (RESOLVED 2026-05-11 via F6 (b) variant — DEC-PM-3 v2): ECHO가 ammo 0이 된 직후 사망 → 복원 시 `restore_idx` (= rewind_trigger 직전 9프레임) 시점의 ammo로 회복. `PlayerSnapshot`이 `ammo_count: int`를 per-tick 캡처 (8번째 PM-노출 필드, ADR-0002 Amendment 2). Pillar 1 contradiction 해소. ~~Weapon GDD의 (a)/(b) 양자택일 의무는 폐기~~ — 정책 락인됨. Weapon GDD #7는 Amendment 2 implementation (ammo_count write-into-snapshot mechanism + Weapon-side restoration hook on `rewind_completed`) 의무만 남음.
...
| # | 시스템 | Hard/Soft | 데이터 흐름 | 의존 본질 |
|---|---|---|---|---|
| **#11** | Boss Pattern | Hard (토큰 경제) | `boss_killed(boss_id)` → TRC가 `grant_token()` 호출 | 토큰 보충 단일 메커니즘 |
...
| **#3** | [Camera System](camera.md) | Hard | `rewind_started()` + `rewind_completed(player: PlayerMovement, restored_to_frame: int)` 시그널 | Camera는 `rewind_started`로 freeze (`is_rewind_frozen=true` → R-C1-1 skip — Pillar 1 "시선은 되감기지 않는다" literal); `rewind_completed`로 unfreeze + shake 강제 클리어 + position 재유도 + `reset_smoothing()` 호출 (camera.md R-C1-8/9 + F.1 row #5 reciprocal). DT-CAM-1 0 px drift 자명한 검증 경로. Camera #3 Approved 2026-05-12 RR1 PASS. |
| **#4** | [Audio System](audio.md) | Hard | `rewind_started(remaining_tokens: int)` + `rewind_completed(player: Node2D, restored_to_frame: int)` 시그널 | AudioManager: `rewind_started` 수신 시 Music 버스 −6 dB duck (2프레임 Tween) + `sfx_rewind_activate_01.ogg` 재생; `rewind_completed` 수신 시 Music 버스 0 dB 복원 (10프레임 linear Tween). audio.md Rules 8/9 + F. Upstream #9. Audio #4 Approved 2026-05-12. |
...
| #6 Player Movement | `restore_from_snapshot()`, 8-필드 PM-노출 (7 PM-owned + 1 Weapon-owned `ammo_count` per ADR-0002 Amendment 2), `_is_restoring: bool` 가드, anim method-track 핸들러의 `_is_restoring` 체크 | C.3, I9 |
| #8 Damage | `player_hit_lethal(cause: StringName)` 발행, `lethal_hit_detected`/`death_committed` 분리, 모든 hazard 동일 시그널 | E-11 |
| #11 Boss Pattern | `boss_killed(boss_id: StringName)` 발행, phase 전이 rewind-immunity 명시, `process_physics_priority = 10` | C.3, E-18 |
...
> **Terminology** (canonical mapping — W4 fix `/review-all-gdds` 2026-05-11; mirrors `player-movement.md` C.1.3): **8 PM-noted = 7 PM-owned + 1 Weapon-noted**. *PM-noted* (PM-readable) = 8 fields that PM exposes via `PlayerSnapshot` schema (7 PM-owned single-writer + `ammo_count` Weapon-owned single-writer per Amendment 2). *PM-owned* (PM single-writer) = 7 fields. *Resource total* = 9 fields = 8 PM-noted + 1 TRC-internal `captured_at_physics_frame` (Amendment 1). Bare "PM-노출" in this GDD always means *PM-noted* (8). AC-A1 round-trip identity asserts the 8 PM-noted fields restore bit-identically; `captured_at_physics_frame` is TRC-internal and not subject to round-trip.

- **AC-A1** [AUTO] GIVEN 세션이 W(90) 물리 프레임 이상 진행, WHEN `_physics_process` 발화, THEN `_capture_to_ring()`이 다음 3-tier ownership 패턴으로 슬롯을 완성한다 (W3 fix `/review-all-gdds` 2026-05-11 — single-writer 계약 명시화): (a) **PM 7-필드 read** (`global_position`, `velocity`, `facing_direction`, `animation_name`, `animation_time`, `current_weapon_id`, `is_grounded` — `forbidden_pattern direct_player_state_write_during_rewind` 준수; TRC는 PM 노드에 *쓰기* 금지, read만), (b) **WeaponSlot.ammo_count read** (Amendment 2 2026-05-11 Weapon single-writer per ADR-0002; TRC는 Weapon 노드에 *쓰기* 금지), (c) **TRC self-set `captured_at_physics_frame`** (Amendment 1 — TRC가 단일 writer인 유일한 필드). 총 9-필드가 `_ring[_write_head]`에 기록되고 `_write_head = (_write_head + 1) % 90`을 같은 틱에 진행. Test fixture spy: TRC가 PM/Weapon 노드 자체 상태를 변경하지 않음을 검증 (assertion = `player.ammo_count_before == player.ammo_count_after` 매 tick). _Rule 1 / Formula D3 / ADR-0001 7-PM-owned + Amendment 2 Weapon-owned + Amendment 1 TRC-internal 단일 출처 검증._
...
- **AC-A4** [AUTO] GIVEN `try_consume_rewind()` 가 `true` 반환, WHEN `restore_from_snapshot(snap)` + Weapon-side restoration (OQ-PM-NEW orchestration) 완료, THEN player 노드의 `global_position`, `velocity`, `facing_direction`, `current_weapon_id`, `animation_name`, `animation_time`, `is_grounded` 모두 PM-side 복원으로 `snap` 값과 일치 AND `WeaponSlot.ammo_count == snap.ammo_count` (Weapon-side restoration). `AnimationPlayer.seek(snap.animation_time, true)` 호출됨. _Rule 10 / ADR-0001 8 PM-노출 필드 계약 검증 (Amendment 2 2026-05-11 — ammo_count Weapon-owned)._
...
- **AC-B2** [AUTO] GIVEN `grant_on_boss_kill=true`, WHEN `boss_killed` 시그널 발화, THEN `get_remaining_tokens()` 정확히 1 증가, `token_replenished` 새 합계로 emit. _Rule 15 / Formula D2 검증._
- **AC-B3** [AUTO] GIVEN `get_remaining_tokens() == max_tokens`(기본 5), WHEN `boss_killed` 재발화, THEN `get_remaining_tokens()`이 `max_tokens` 초과 안 함. _Rule 3 / `RewindPolicy.max_tokens` cap 검증._
...
| 8-field PM-노출 restore correctness (per ADR-0002 Amendment 2) | Unit (GUT) | `tests/unit/time-rewind/` |
...
|---|---|---|---|
| ~~**OQ-1**~~ | ~~Ammo 카운트 복원 정책 — (a) "resume with live ammo" 비준 vs (b) `PlayerSnapshot.ammo_count` 추가 ADR amendment? (E-22, F6)~~ — **RESOLVED 2026-05-11 (b) variant** via player-movement.md DEC-PM-3 v2 / B5 Pillar 1 resolution. ADR-0002 Amendment 2 obligatory. Player Shooting #7는 Weapon-side write/restoration mechanism (OQ-PM-NEW) 결정 의무 잔여. | ~~Weapon GDD (#7)~~ | ✅ **2026-05-11 closed** |
...
| ~~**OQ-4**~~ ✅ **Resolved 2026-05-11 (Scene Manager #2 Approved RR7)** | ~~`scene_will_change()` 시그널이 정확히 언제 emit되는가 — 트리거 진입 시점 vs unload 시작 시점? 토큰은 무조건 보존? (E-16)~~ → **Emit 타이밍 = `change_scene_to_packed` 호출 직전 동일 물리 틱 sync emit** (scene-manager.md C.1 Rule 4 sole-producer); 토큰은 항상 보존 (scene-manager.md C.1 Rule 7 + D.2 invariant — SM has zero `_tokens` write sites). F.1 row #2 `*(provisional)*` 제거 + canonical wiring locked. | [`design/gdd/scene-manager.md`](scene-manager.md) C.1 Rules 4+7 + D.2 + D.6 | **Resolved** |
...
| ~~**OQ-15**~~ ✅ **Resolved 2026-05-11** | ~~Tier 3 Input Remapping에서 `"rewind_consume"` 액션이 chord 매핑 가능한지?~~ → **`default_only` interpretation locked** (input.md C.2 단일 출처): "single-button no-chord" invariant은 *shipped default mapping*에 한정; Tier 3 #23 remap UI는 player-bound chord 허용 (1-frame latency advisory 표시 의무). Anti-Pillar #6 (Tier 3 accessibility) 정합. | [`design/gdd/input.md`](input.md) C.2 + Input #1 F.4.1 #9 closure | **Resolved** |
