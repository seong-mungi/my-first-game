...
4. ECHO에 치명타 명중 시 State Machine(System #5)은 즉시 `ALIVE → DYING` 전이를 발생시키고 `RewindPolicy.dying_window_frames` 프레임(기본 12 / 0.2s) grace window를 연다. Damage 시스템은 사망 *결정*을 같은 길이만큼 지연한다. **`player_hit_lethal` 시그널 수신 즉시 TRC는 `_lethal_hit_head: int = _write_head`를 캐시한다** — 이 동결 값이 향후 `restore_idx` 계산의 기준이 되며, 이는 DYING grace 동안 캡처가 계속 진행되어도 복원 시점이 *사망 직전*에 고정되도록 보장한다 (systems-designer 검증 2026-05-09 — ADR-0002 amendment 동반).
...
17. **Lethal-hit latch**: SM은 `_lethal_hit_latched: bool` 플래그를 소유한다. `ALIVE → DYING` 전이 시 `true`로 설정, `REWINDING → ALIVE` 또는 `DEAD` 전이 시 `false`로 클리어. 래치가 `true`인 동안 추가 `player_hit_lethal` 시그널은 SM이 무시한다. 연속 데미지(산성 풀, 레이저 sweep)와 같은 틱 다중 치명타가 `_lethal_hit_head`를 재캐시하는 것을 차단한다 (E-12, E-13).
...
16. `boss_defeated`와 `player_hit_lethal`이 같은 물리 틱에 도착하는 경우 — Boss(`process_physics_priority = 10`)가 Damage 처리 이전에 `grant_token()`을 발행하도록 process priority 사다리를 고정한다. 토큰 손실 0 보장.
...
| #6 | Player Movement *(provisional)* | TRC가 매 틱 PlayerMovement 속성을 *읽기*; `restore_from_snapshot()`을 *쓰기 단일 경로*로 호출. | 캡처 (read): `global_position: Vector2`, `velocity: Vector2`, `facing_direction: int`, `current_animation_name: StringName`, `current_animation_time: float`, `current_weapon_id: int`, `is_grounded: bool`. 복원 (write): `func restore_from_snapshot(snap: PlayerSnapshot) -> void`. Forbidden pattern `direct_player_state_write_during_rewind` 적용. | **Hard** |
...
| **I2** | `boss_defeated`와 `player_hit_lethal` 같은 틱 중첩 시 토큰 손실 | Boss process priority 10 → Damage 처리 전 `grant_token()` 발행 보장. process priority 사다리 (player=0, TRC=1, enemies=10, projectiles=20). |
...
| `H_lethal` | int | 0–89 | `_lethal_hit_head` — `player_hit_lethal` 시 동결된 H 사본 |
...
T_after_boss_kill = T + 1                      -- on grant_token() (when grant_on_boss_kill)
...
```
t_capture_per_tick  ≈ N_fields × t_field_copy = 8 × ~6 ns ≈ 50 ns   (write-into-place)
t_restore_one_time  ≈ N_fields × t_field_copy + t_anim_seek
                    ≈ 8 × 6 ns + 200 μs ≈ 200 μs                    (AnimationPlayer.seek dominate)
frame_budget        = 16,600 μs (1 / 60 fps)
rewind_subsys_cap   = 1,000 μs  (≤1 ms envelope, technical-preferences.md)
capture_fraction    = 50 ns / 16,600 μs ≈ 0.0003% per tick
restore_fraction    = 200 μs / 16,600 μs ≈ 1.2% (단발성, rewind 발동 프레임만)
```
...
| `t_field_copy` | ns | ~6 | GDScript 단일 필드 copy 추정 |
...
**Example**: 정상 플레이 60fps에서 캡처 비용 = 50 ns × 60 = 3 μs/sec → 전체 CPU 시간의 0.018%. 1회 rewind 발동: 발동 프레임에 200 μs, ≤ 1 ms 봉투 만족.
...
- **E-06** boss_defeated와 player_hit_lethal 같은 틱 → boss process priority=10이 Damage 이전 발화 (Rule 16)
...
> 33개 기준을 A–G 7군으로 분류. 자동화는 [AUTO], 수동 테스트는 [MANUAL].
...
- **AC-A3** [AUTO] GIVEN `player_hit_lethal`이 frame F에 발화, WHEN DYING grace 윈도우(F~F+11) 어느 시점이든 `try_consume_rewind()` 호출, THEN `restore_idx == (_lethal_hit_head − 9 + 90) % 90`이며 `_lethal_hit_head`는 frame F의 `_write_head` 동결값(호출 시점 값 아님). _Rule 9 / Formula D1 / ADR-0002 Amendment 1 검증._
...
- **AC-B5** [AUTO] GIVEN test harness에서 같은 물리 프레임에 `boss_defeated`와 `player_hit_lethal` 주입(Boss `process_physics_priority=10`, Damage 더 낮은 priority 노드), WHEN 시그널 처리, THEN signal log에서 `token_replenished`가 `token_consumed` *이전* 관측, 두 이벤트 후 `get_remaining_tokens()`이 이벤트 전 값과 동일(grant + consume = net zero). _Rule 16 / Integration Risk I2 검증._
...
- **AC-C2** [AUTO] GIVEN ECHO DYING AND 12프레임 경과 with no valid rewind input, WHEN frame 13 도착, THEN SM이 DEAD 전이. _Rule 8 / `dying_window_frames` 기본 12 검증._
...
- **AC-D4** [AUTO] GIVEN ECHO DYING 동안 보스 페이즈 전환(HP 임계 통과), WHEN SM이 전환 처리, THEN 보스 페이즈 rewind 발생 안 함 — ECHO 복원 후에도 보스 `_phase`가 새 값 유지. _E-18 / ADR-0001 Player-only scope 검증._
- **AC-D5** [AUTO] GIVEN ECHO mid-shooting-animation 중 `restore_from_snapshot()` 호출, WHEN `AnimationPlayer.seek()` 발화로 method-track key (예: `_on_anim_spawn_bullet`)가 평소대로면 트리거 시점, THEN 발사체 spawn 안 됨 — `PlayerMovement._is_restoring == true`이기 때문. `_is_restoring`은 다음 `_physics_process`에서 `false` 복원. _E-21 / Integration Risk I9 검증._

### E. Performance

- **AC-E1** [AUTO] GIVEN 60 Hz 게임 + TRC 활성, WHEN `_capture_to_ring()` 비용을 600 연속 프레임 동안 Godot Profiler 또는 test-harness 타이머로 샘플링, THEN per-tick 평균 ≤ 16.6 ms 프레임 budget의 0.05% (≤ 8.3 μs/tick). _Formula D6 / technical-preferences.md 시간 되감기 서브시스템 cap 검증._
