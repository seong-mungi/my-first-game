...
| 7 | `shoot` | edge or hold (#7 결정) | T1 | Player Shooting #7 | Phase 2 | PM은 read 안 함 |
...
2. **규칙 2 (콜백 binding 금지)**: `_input` / `_unhandled_input` / `_unhandled_key_input` 콜백에 게임플레이 로직(움직임/사격/시간되감기) binding 금지. *예외*: UI/Menu(#18) + PauseHandler 노드(C.1.4) 한정.
...
func _unhandled_input(event: InputEvent) -> void:
    ...
    get_viewport().set_input_as_handled()   # consume on every decision (resume / initiate / veto) — Godot 4.6 canonical "consume on decision" pattern; AC-IN-20-A + B4 reconciliation 2026-05-11
```
...
**Consumption invariant (B4 reconciliation 2026-05-11)**: PauseHandler MUST call `set_input_as_handled()` on **every** decision path — resume, initiate, AND veto. Rationale: (a) Godot 4.6 canonical pattern is consume-on-decision (any node that resolves the event should consume it to prevent downstream misclassification); (b) ActiveProfileTracker._input fires *before* `_unhandled_input` regardless (E-IN-NEW), so consumption here doesn't block profile classification — but it does prevent any other `_unhandled_input` listener from acting on a resolved pause event; (c) AC-IN-20-A asserts both veto and resume call this — earlier draft (Session 12) only consumed on success, creating a 4-way contradiction (qa-lead BLK-1 + godot-specialist Item 1 + gameplay-programmer BLOCKING-1 + main-review). CD adjudication: veto SHOULD consume.
...
Tier 1 튜토리얼 텍스트 0줄(Pillar 4) 약속 하에서, *KB+M `Shift = rewind_consume` 발견 위험* (Katana Zero 미경험 플레이어가 0.2s DYING grace 내 Shift 본능 미발화)을 완화하는 *single instance, context-timed* 힌트.

- **트리거**: 세션 내 *최초* `ALIVE → DYING` 전이. SM이 `first_death_in_session` 신호 emit (1-time latch); HUD #13 구독.
- **표시 시간**: DYING grace window (12 frames = 200ms). REWINDING/DEAD 전이 시 즉시 사라짐. *세션 내 재출현 없음*.
...
**HUD #13 책임**: 비주얼 렌더링, 12프레임 페이드, 위치/크기/색.
**SM 책임**: `first_death_in_session` 신호 owner (1회 latch + scene change reset O6).

**Cross-doc obligations (F.4.1)**:
1. HUD #13 GDD: prompt 시각 시방 + 12프레임 fade 시방 포함.
2. SM #5: `first_death_in_session` 신호 추가 (1-time latch + scene clear).
...
| `aim_lock` | F | WASD home row 우측 ring finger; non-Shift (Shift conflict resolution) |
...
| `aim_lock` | RB (Right Bumper) | Cuphead lock-aim precedent; right shoulder (LT는 rewind 점유, 좌-우 분리) |
...
`InputEvent.timestamp` 필드(Godot `InputEvent.device` + `InputEvent.timestamp`)는 *wall-clock 밀리초* 값이다. ADR-0003 `determinism_clock` 결정에 따라 게임플레이 로직에서 사용 금지. 모든 frame 참조는 `Engine.get_physics_frames()` 단일 출처.

```gdscript
# 금지 — wall-clock 의존
if Time.get_ticks_msec() − event.timestamp < 100:  # ← BANNED
...
| 후보 | 단일 출처 | 비고 |
|---|---|---|
| LT chatter near threshold 0.5 → 재발화 처리 | state-machine.md C.5 (`_trigger_held` gate) | Input은 `is_action_just_pressed` 정상 발화만 보장 — 후속 처리는 SM 책임 |
...
| #7 Player Shooting | **Hard** *(provisional)* | T1 | `is_action_just_pressed("shoot")` or `is_action_pressed("shoot")` (#7 결정). | #7 GDD 작성 시 SHOOT detect mode 결정 + 본 GDD C.1.1 row 7 update (F.4.2 #1) |
...
| # | 대상 GDD | 의무 | 사유 |
|---|---|---|---|
| O-IN-1 | state-machine.md C.5 (신규 sub-section) | `_trigger_held: bool` gate 추가 — `JOY_AXIS_TRIGGER_LEFT` chatter 시 `is_action_just_pressed`가 동일 hold 동안 한 번만 처리되도록. `_rewind_input_pressed_at_frame` 갱신 직전 가드. | E-IN-2 + Steam Deck 1세대 LT 마모 시 chatter 위험 (gdscript-specialist 검증) |
...
| O-IN-3 | state-machine.md (신규 signal) | `first_death_in_session(profile: StringName)` 신호 owner — 세션 내 *최초* `ALIVE → DYING` 전이 시 1-time latch emit. Scene change(O6)에서 latch reset. | C.1.5 first-death hint 구현 |
...
13. **docs/registry/architecture.yaml** → 신규 항목 4건 추가:
    ...
    - `forbidden_patterns.gameplay_input_in_callback` + `wall_clock_in_input_logic` + `deadzone_in_consumer` (3 신규)
...
**AC-IN-02** *(Logic GUT — BLOCKING)* — InputActions const class has exactly 9 entries matching InputMap.
GIVEN `InputActions` class is loaded.
WHEN all const members are enumerated.
THEN count == 9 AND for each const value v: `InputMap.has_action(v) == true`.
_cross-ref: C.4._

### H.2 Polling Discipline (C.1.2)

**AC-IN-03** *(Logic GUT — BLOCKING)* — Cascade invariant: 3 consumers read same action state in same tick.
GIVEN a `cascade_3_consumer_harness` scene instantiated with mock PM, SM, TRC nodes.
WHEN fixture calls `Input.action_press(InputActions.REWIND_CONSUME)` then `await get_tree().physics_frame`.
THEN PM.last_rewind_poll == SM.last_rewind_poll == TRC.last_rewind_poll == true in that tick. No consumer reads a divergent value.
**Deliverable**: write `tests/helpers/cascade_3_consumer_harness.gd`.
_cross-ref: C.1.2 Rule 1 + B.2 Cascade + ADR-0003._
...
**AC-IN-13** *(Logic GUT — BLOCKING)* — `first_death_in_session` signal fires exactly once per session.
GIVEN SM mock with 1-time latch on `first_death_in_session`.
WHEN ALIVE→DYING transition fires twice in the same session.
THEN `first_death_in_session` signal received exactly once by HUD spy. Second transition emits 0 additional signals.
_cross-ref: C.1.5._
...
GIVEN `tests/fixtures/input_map_default_snapshot.tres` golden resource (KB+M C.3.1 + Gamepad C.3.2).
...
**Deliverable**: write `tests/fixtures/input_map_default_snapshot.tres`.
_cross-ref: C.3.1 + C.3.2._
...
GIVEN `ActiveProfileTracker` with deterministic `_last_input_source` / `_last_input_frame` override via `tests/helpers/active_profile_mock.gd`.
...
**AC-IN-20-A** *(Logic GUT — BLOCKING)* — `PauseHandler` calls `set_input_as_handled()` on both veto and resume paths.
...
**AC-IN-20** *(Manual — ADVISORY)* — F.4.1 cross-doc obligation batch fully applied before story closure.
GIVEN all 13 F.4.1 items are tracked in `production/qa/input-f41-batch.md`.
WHEN the Input System story is marked Done.
THEN all 13 checklist items are checked; git diff against the 4 target GDD files (player-movement.md, state-machine.md, time-rewind.md, systems-index.md) + `architecture.yaml` shows the expected provisional-flag removals, naming unifications, and registry additions.
...
| H.10 PauseHandler Input Consumption | 1 | 1 | 0 |
| H.11 F.4.1 Ledger | 1 | 0 | 1 |
| **Total** | **20** | **19** | **1** |

### H.13 Tooling Deliverables (consolidated)

본 GDD AC가 강제하는 신규 테스트 인프라 + CI 스크립트 — F.4.1 #14 batch에 포함:

1. `tests/helpers/input_action_press_helper.gd` (AC-IN-19)
2. `tests/helpers/input_event_inject_helper.gd` (AC-IN-19)
3. `tests/helpers/cascade_3_consumer_harness.gd` (AC-IN-03)
4. `tests/helpers/active_profile_mock.gd` (AC-IN-17)
5. `tests/fixtures/input_map_default_snapshot.tres` (AC-IN-01, AC-IN-15)
6. `tools/ci/input_action_static_check.sh` (AC-IN-04)
7. `tools/ci/_input_purity.py` + `tools/ci/input_callback_purity_check.sh` (AC-IN-05)
8. `tools/ci/deadzone_consumer_check.sh` (AC-IN-08)
9. `tools/ci/wall_clock_check.sh` (AC-IN-18)
10. `tools/ci/run_static_checks.sh` **meta-runner** — *PM B2 vaccine* (AC-IN-04)
11. `production/qa/input-f41-batch.md` checklist (AC-IN-20)
...
| Gamepad `aim_lock` = **RB** (Cuphead precedent) | 2026-05-10 | ux-designer | LT는 rewind 점유, 좌-우 분리 |
