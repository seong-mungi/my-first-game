...
| B3 | Pillar | game-designer | First-Death Hint returns text string. game-concept Pillar 4 = "0 lines of text tutorial". Undeclared carve-out. Either declare Pillar 4 exception in C.1.5 + Decision Log, or change deliverable to `Texture2D` glyph (aligns with ux-designer REC-2 `[LT] ↺`). |
...
| B6 | Code/Spec | 2-way: ux-designer + game-designer | F-key `aim_lock` justification "WASD home row right ring finger" (C.3.1 line 193 + A.1 line 851) is anatomically wrong — F is left **index** finger reach. Real argument: Hotline Miami Shift-aim muscle-memory separation. Fix documentation, keep keymap. |
...
| B9 | Formula | systems-designer | D.3 radial composite deadzone formula entirely missing. C.1.3 line 271 declares "single source" with no math. Add D.3 with variable table + worked examples (Echo case T=0.2). |
...
| B14 | Cross-doc | gameplay-programmer | O-IN-1 (`_trigger_held` gate) adds new state to **Approved** state-machine.md. SM directive says Round 3 blocked except cross-doc-contradiction exception. Sprint sequencing blocker — must either invoke exception or re-review SM; no owner assigned. |
...
- B.4 "Achievers primary" framing may not be Pillar-1-consonant (learning tool ↔ mastery tension). Tier 2 playtest target audience validation.
...
| B14 | O-IN-1 `_trigger_held` adds state to Approved SM | F.4.1 #4 marked ✅ — applied to SM C.2.2 O9 under Round-7 cross-doc-contradiction exception (active.md Session 14 header umbrella); SM "8 obligations" → "9 obligations" |
