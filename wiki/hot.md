...
0. [[Research 8-Way Aim Usability For Run-and-Gun]] — **NEW LATEST**. 런앤건 8방향 조준 사용성 + Echo Input #1 갱신 권고 6항
0b. [[Bot Validation Catalog Summary]] — INDEX. 14 페이지 단일 진입점 + 5 황금률 + 솔로 개발자 로드맵 + 비협상 결정 8종
...
5. [[Non-Boss Bot Validation Suites]] — 4 비-보스 스위트 + 무기-vs-적 매트릭스 자동 생성
6. [[Bot Validation Pipeline Architecture]] — B+C+D 풀스택 (RL 브릿지 + 대시보드 CI + GDD 루프)
7. [[Bot Human Validation Reconciliation]] — 봇 verdict ↔ 인간 플레이테스트 4사분면 매트릭스
8. [[Determinism Verification Replay Diff]] — CI 결정론 게이트 + Godot 4.6 footgun 10종
9. [[Death Heatmap Analytics]] — 공간/시간/패턴 3 뷰 + DBSCAN + 안전지대 검출
10. [[Heuristic Bot Reaction Lag Simulation]] — 9프레임 인간 반응 지연 = Echo rewind 윈도우 = 봇 fairness proxy
11. [[GDD Bot Acceptance Criteria Template]] — GDD 8.2 봇 검증 YAML 표준
12. [[RL Reward Shaping For Deterministic Boss]] — Echo PPO 보상 함수 + 커리큘럼 + reward hacking 5종
13. [[Deterministic Game AI Patterns]] — 결정론 게임 AI 활용 4 zone + Echo Tier 1-3 권고
14. [[AI Playtest Bot For Boss Validation]] — 4 봇 아키타입 + 메트릭 + 시간메커닉 검증 시나리오 4종
15. [[Time Manipulation Run and Gun]] — PoP/Braid/Katana Zero 3 모델 + Echo 선택 가이드 (고스트 + speedrun 베이스라인)
...
- **[2026-05-12]** 📦 **CCGS Context Bloat 진단/처방 카탈로그화** — Echo systems-index.md 52 KB 실측, 활성/이력 분리 5 황금률 + Tier 1-4 처방. 즉시 액션 (Status 컬럼 truncate + "Last Updated" 정리)으로 systems-index 90% 절감 가능. CCGS upstream 기여 권고 4항. (참고: [[CCGS Context Bloat Remediation]])
- **[2026-05-11]** 🎯 **8방향 조준 입력 시스템 검증 완료 (autoresearch 6 페이지)** — Echo Input #1 GDD 갱신 권고 6항: (1) FACING_THRESHOLD 0.10→0.15 (Steam Deck 드리프트 회피), (2) 각도 hysteresis ±4°, (3) 2-3 프레임 sector commitment timer, (4) KB+M aim_lock=F 유지 (Shift/RMB/Q-E 회피), (5) Gamepad aim_lock=RB 유지 (Cuphead/Galuga 컨벤션), (6) 4-tier aim assist + XAG 107 준수 (single-stick + auto-fire + ±50% sensitivity + 어시스트 ↔ 난이도 직교). (참고: [[Research 8-Way Aim Usability For Run-and-Gun]] · [[Aim Lock Modifier Pattern]] · [[Analog Stick To 8-Way Quantization]] · [[Aim Assist Accessibility Tiers]])
